...
def _write_dhcpcd_config(settings: NetworkSettings) -> None:
    """Persist network settings to dhcpcd configuration for RPi environments."""

    lines = ["# Generated by ONVIF NVR device management", ""]
    lines.append(f"interface {settings.interface}")

    if settings.mode == NetworkMode.static:
        prefix = _netmask_to_prefix(settings.subnet_mask or "255.255.255.0")
        ip_with_prefix = f"{settings.static_ip}/{prefix}" if settings.static_ip else None
        if ip_with_prefix:
            lines.append(f"static ip_address={ip_with_prefix}")
        if settings.gateway:
            lines.append(f"static routers={settings.gateway}")
        if settings.dns_servers:
            lines.append(
                "static domain_name_servers=" + " ".join(settings.dns_servers)
            )
    else:
        lines.append("# DHCP enabled - no static overrides")

    content = "\n".join(lines) + "\n"
    _write_file(Path("/etc/dhcpcd.conf"), content)
